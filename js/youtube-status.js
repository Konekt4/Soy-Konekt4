(function () {
    const statusEls = Array.prototype.slice.call(document.querySelectorAll('.youtube-status'));

    if (!statusEls.length) {
        return;
    }

    const defaultChannelId = 'UCosNVo3F769-BqIX9UcJb0A';
    ...
    function getFeed(apiUrl) {
        if (!feedCache.has(apiUrl)) {
            ...
            feedCache.set(apiUrl, request);
        }

        return feedCache.get(apiUrl);
    }

    function updateStatus(statusEl) {
        const channelId = statusEl.dataset.channelId || defaultChannelId;
        const feedUrl = `https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`;
        const apiUrl = `https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(feedUrl)}`;
        const fallbackVideoPage = statusEl.dataset.fallback || statusEl.href || 'https://www.youtube.com/@konekt4/videos';
        ...
        getFeed(apiUrl)
            .then(function (data) {
                ...
                const allowShorts = statusEl.dataset.allowShorts === 'true';
                const latestVideo = data.items.find(function (item) {
                    return (allowShorts || !isLikelyShort(item)) && !isTaggedStream(item);
                });
    ...
    statusEls.forEach(updateStatus);
})();
